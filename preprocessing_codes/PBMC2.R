...
# Load data
```{r message = FALSE}
fastqdirs <- list.files("~/Documents/Zurich/single_cell/dane/PBMC", full.names = TRUE)
names(fastqdirs) <- basename(fastqdirs)
fastqdirs <- fastqdirs[file.exists(paste0(fastqdirs, "/matrix.mtx"))]
fastqdirs
sce <- DropletUtils::read10xCounts(fastqdirs)
meta <- read_excel("~/Documents/Zurich/single_cell/dane/PBMC/EXP_CR005 Samples ID.xlsx")
sce$Sample<-gsub("/Users/joannazyprych/Documents/Zurich/single_cell/dane/PBMC/","",sce$Sample)
...
seurat@meta.data$cluster.main <- seurat@ident
```
